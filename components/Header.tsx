import Link from 'next/link';
import { useEffect, useState } from 'react';
import Image from 'next/image';
import { motion } from 'framer-motion';

export default function Header() {
  const [scrolled, setScrolled] = useState(false);
  useEffect(() => {
    const onScroll = () => setScrolled(window.scrollY > 50);
    window.addEventListener('scroll', onScroll);
    return () => window.removeEventListener('scroll', onScroll);
  }, []);
  return (
    <>
      {/* Skip Navigation Links for Screen Readers */}
      <a
        href="#main-content"
        className="sr-only focus:not-sr-only focus:absolute focus:top-4 focus:left-4 focus:z-50 focus:px-4 focus:py-2 focus:bg-molten focus:text-black focus:rounded focus:font-bold"
        tabIndex={1}
      >
        Skip to main content
      </a>
      <a
        href="#navigation"
        className="sr-only focus:not-sr-only focus:absolute focus:top-4 focus:left-32 focus:z-50 focus:px-4 focus:py-2 focus:bg-molten focus:text-black focus:rounded focus:font-bold"
        tabIndex={2}
      >
        Skip to navigation
      </a>

      <header
        className={scrolled ? 'header scrolled' : 'header'}
        role="banner"
        aria-label="Site header with navigation and social links"
      >
        <div className="flex flex-col w-full">
          {/* Top row: Logo, Profile Picture, and Social Icons */}
          <div className="flex items-center justify-between w-full pb-1">
            <div className="flex items-center gap-3">
              <Link
                href="/home"
                className="logo"
                aria-label="CLB Consulting - Return to homepage"
              >
                <Image
                  src="/images/Purple Logo.png"
                  alt="CLB Consulting Logo"
                  width={120}
                  height={32}
                  className="h-8 w-auto"
                  priority
                />
              </Link>
              <a
                href="https://lizpettengill.com"
                target="_blank"
                rel="noopener noreferrer"
                className="designer-credit"
                aria-label="Visit Liz Pettengill's website - Logo designer"
              >
                Logo by Liz Pettengill
              </a>
            </div>

            {/* Profile Picture - Desktop Only */}
            <div className="hidden md:block absolute left-1/2 transform -translate-x-1/2">
              <div className="w-14 h-14 rounded-full border border-molten/50 overflow-hidden bg-gradient-to-br from-molten/30 to-gray-700/50 relative">
                <Image
                  src="/images/profile/chris-profile.jpg"
                  alt="Chris Lee Bergstrom"
                  width={56}
                  height={56}
                  className="w-full h-full object-cover object-top"
                  priority
                  onError={(e) => {
                    const target = e.target as HTMLElement;
                    target.style.display = 'none';
                    const fallback = target.nextElementSibling as HTMLElement;
                    if (fallback) fallback.style.display = 'flex';
                  }}
                />
                <div className="w-full h-full flex items-center justify-center absolute inset-0" style={{ display: 'none' }}>
                  <span className="text-molten text-xs font-bold">CB</span>
                </div>
              </div>
            </div>

            <nav
              className="flex items-center space-x-2 md:space-x-4"
              role="navigation"
              aria-label="Social media links"
            >
              <a
                href="https://www.instagram.com/chrisleebergstrom"
                target="_blank"
                rel="noopener noreferrer"
                className="social-icon"
                aria-label="Follow Chris Lee Bergstrom on Instagram"
              >
                <svg className="w-5 h-5 md:w-6 md:h-6" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                  <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.40z" />
                </svg>
              </a>
              <a
                href="https://www.facebook.com/share/15a8S2BF9S/?mibextid=wwXIfr"
                target="_blank"
                rel="noopener noreferrer"
                className="social-icon"
                aria-label="Follow Chris Lee Bergstrom on Facebook"
              >
                <svg className="w-5 h-5 md:w-6 md:h-6" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                  <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                </svg>
              </a>
              <a
                href="https://www.linkedin.com/in/chris-bergstrom"
                target="_blank"
                rel="noopener noreferrer"
                className="social-icon"
                aria-label="Connect with Chris Lee Bergstrom on LinkedIn"
              >
                <svg className="w-5 h-5 md:w-6 md:h-6" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                  <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
                </svg>
              </a>
              <a
                href="https://www.youtube.com/@chrisleebergstrom"
                target="_blank"
                rel="noopener noreferrer"
                className="social-icon"
                aria-label="Subscribe to Chris Lee Bergstrom on YouTube"
              >
                <svg className="w-5 h-5 md:w-6 md:h-6" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                  <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z" />
                </svg>
              </a>
              <a
                href="https://github.com/Nxtvanhalen"
                target="_blank"
                rel="noopener noreferrer"
                className="social-icon"
                aria-label="View Chris Lee Bergstrom on GitHub"
              >
                <svg className="w-5 h-5 md:w-6 md:h-6" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                  <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
                </svg>
              </a>
            </nav>
          </div>

          {/* Primary Navigation Links */}
          <nav
            id="navigation"
            className="flex items-center justify-between max-w-4xl mx-auto w-full px-4 py-0.5 border-t border-molten/20 md:px-8"
            role="navigation"
            aria-label="Main navigation"
          >
            <motion.a
              href="/about"
              className="nav-link"
              aria-label="Learn about Chris Lee Bergstrom's background and expertise"
              whileHover={{ scale: 1.1, color: "#ffffff" }}
              whileTap={{ scale: 0.95 }}
            >
              About Chris
            </motion.a>
            <motion.a
              href="/projects"
              className="nav-link"
              aria-label="View Chris's portfolio and project gallery"
              whileHover={{ scale: 1.1, color: "#ffffff" }}
              whileTap={{ scale: 0.95 }}
            >
              Projects
            </motion.a>
            <motion.a
              href="/news"
              className="nav-link"
              aria-label="Read latest news and press coverage"
              whileHover={{ scale: 1.1, color: "#ffffff" }}
              whileTap={{ scale: 0.95 }}
            >
              News/Press
            </motion.a>
          </nav>

          {/* Secondary Navigation Links */}
          <nav
            className="flex items-center justify-center gap-12 max-w-2xl mx-auto w-full px-4 py-1 md:px-8"
            role="navigation"
            aria-label="Secondary navigation"
          >
            <motion.a
              href="/faq"
              className="nav-link"
              aria-label="Frequently asked questions about AI consulting services"
              whileHover={{ scale: 1.1, color: "#ffffff" }}
              whileTap={{ scale: 0.95 }}
            >
              FAQ
            </motion.a>

            <motion.a
              href="https://ryder-k6er.onrender.com"
              target="_blank"
              rel="noopener noreferrer"
              className="nav-link ml-7"
              style={{
                color: '#00ff00',
                textShadow: '0 0 8px #00ff00, 0 0 4px #00ff00'
              }}
              aria-label="Ryder AI Assistant"
              whileHover={{ scale: 1.1, textShadow: '0 0 12px #00ff00, 0 0 8px #00ff00' }}
              whileTap={{ scale: 0.95 }}
            >
              Ryder
            </motion.a>

            <motion.a
              href="/blog"
              className="nav-link"
              aria-label="Read strategic musings and insights about AI and consulting"
              whileHover={{ scale: 1.1, color: "#ffffff" }}
              whileTap={{ scale: 0.95 }}
            >
              Musings
            </motion.a>
          </nav>
        </div>
      </header>
    </>
  );
}